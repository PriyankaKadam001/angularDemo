...
import { TableState, SelectItem, PrimeNGConfig } from './global';
import { FilterMetadata,FilterMatchMode, TranslationKeys } from './global';
import { Injectable } from '@angular/core';
import { Subject, Subscription } from 'rxjs';
import { FilterUtils } from './global';
import {trigger,style,transition,animate,AnimationEvent} from '@angular/animations';
import {PrimeTemplate} from './pTemplate';
import {SharedModule} from './pTemplate';
...
export class Table implements OnInit, AfterViewInit, AfterContentInit, OnChanges {
  ...
  @Input() comparfilteredValueeSelectionBy: string = 'deepEquals';
  ...
  ngOnChanges(simpleChange: SimpleChanges) {
      if (simpleChange.value) {
          ...
          if (!this.lazy) {
              ...
              if (this.sortMode == 'single' && this.sortField)
                  this.sortSingle();
              else if (this.sortMode == 'multiple' && this.multiany)
                  this.sortMultiple();
      ...
      if (simpleChange.multiany) {
          this._multiany = simpleChange.multiany.currentValue;
  ...
  @Input() get multiany(): any[] {
      return this._multiany;
  }

  set multiany(val: any[]) {
      this._multiany = val;
  }
  ...
  sort(event) {
      ...
      if (this.sortMode === 'multiple') {
          ...
          let any = this.getany(event.field);

          if (any) {
              if (!metaKey) {
                  this._multiany = [{ field: event.field, order: any.order * -1 }];
              ...
              this._multiany.push({ field: event.field, order: this.defaultSortOrder });
          }
  ...
  sortSingle() {
      if (this.sortField && this.sortOrder) {
          ...
          this.onSort.emit(any);
          this.tableService.onSort(any);
      }
  }
  ...
  sortMultiple() {
      if (this.multiany) {
          if (this.lazy) {
              ...
              if (this.customSort) {
                  this.sortFunction.emit({
                      data: this.value,
                      mode: this.sortMode,
                      multiany: this.multiany
                  });
              }
              else {
                  this.value.sort((data1, data2) => {
                      return this.multisortField(data1, data2, this.multiany, 0);
                  });
          ...
          this.tableService.onSort(this.multiany);
      }
  }

  multisortField(data1, data2, multiany, index) {
      let value1 = ObjectUtils.resolveFieldData(data1, multiany[index].field);
      let value2 = ObjectUtils.resolveFieldData(data2, multiany[index].field);
      let result = null;
      ...
      if (value1 == null && value2 != null)
          ...
          if (value1.localeCompare && (value1 != value2)) {
              return (multiany[index].order * value1.localeCompare(value2));
          }
      }
      else {
      ...
      if (value1 == value2) {
          return (multiany.length - 1) > (index) ? (this.multisortField(data1, data2, multiany, index + 1)) : 0;
      }

      return (multiany[index].order * result);
  }

  getany(field: string) {
      if (this.multiany && this.multiany.length) {
          for (let i = 0; i < this.multiany.length; i++) {
              if (this.multiany[i].field === field) {
                  return this.multiany[i];
              }
          }
      }
...
export class yTableBody implements OnDestroy {
...
export class SortableColumn implements OnInit, OnDestroy {
  ...
  constructor(public dt: Table) {
      if (this.isEnabled()) {
          this.subscription = this.dt.tableService.sortSource$.subscribe(any => {
...
export class SortIcon implements OnInit, OnDestroy {
  ...
  updateSortState() {
      if (this.dt.sortMode === 'single') {
          ...
          let any = this.dt.getany(this.field);
          this.sortOrder = any ? any.order: 0;
      }
...
@NgModule({
  ...
  exports: [Table,SharedModule,SortableColumn,SelectableRow,RowToggler,ContextMenuRow,ResizableColumn,ReorderableColumn,EditableColumn,CellEditor,SortIcon,
          TableRadioButton,TableCheckbox,TableHeaderCheckbox,ReorderableRowHandle,ReorderableRow,SelectableRowDblClick,EditableRow,InitEditableRow,SaveEditableRow,CancelEditableRow,ColumnFilter],
  declarations: [Table,SortableColumn,SelectableRow,RowToggler,ContextMenuRow,ResizableColumn,ReorderableColumn,EditableColumn,CellEditor,yTableBody,ScrollableView,SortIcon,
